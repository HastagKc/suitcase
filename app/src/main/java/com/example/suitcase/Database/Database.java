...
import com.example.suitcase.Model.CatDataModel;
...
public class Database extends SQLiteOpenHelper {
    ...
    public boolean updateUserByEmail(String email, String firstName, String lastName, String newPassword) {
        SQLiteDatabase db = this.getWritableDatabase();
        ContentValues contentValues = new ContentValues();

        // Populate the ContentValues with the new values
        contentValues.put(AUTH_COL_1, firstName);
        contentValues.put(AUTH_COL_2, lastName);
        contentValues.put(AUTH_COL_4, newPassword);

        // Define the WHERE clause to update the specific user based on email
        String whereClause = AUTH_COL_3 + " = ?";
        String[] whereArgs = { email };

        try {
            // Perform the update operation
            int rowsAffected = db.update(AUTH_TABLE, contentValues, whereClause, whereArgs);
